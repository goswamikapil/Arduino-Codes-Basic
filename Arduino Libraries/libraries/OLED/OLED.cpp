...
void OLED::LEDPIN_Init(unsigned char SCL_PIN,unsigned char SDA_PIN,unsigned char RST_PIN,unsigned char DC_PIN)
{
  ...
  pinMode(_datascl,OUTPUT);
  pinMode(_datasda,OUTPUT);
  pinMode(_datarst,OUTPUT);
  pinMode(_datadc,OUTPUT);
}
void OLED::LED_WrDat(unsigned char data)   
{
	unsigned char i = 8;
        digitalWrite( _datadc,HIGH);
        digitalWrite(_datascl,LOW);
	while (i--)
	{
		if (data & 0x80)
		{
			digitalWrite(_datasda,HIGH);;;;
		}
		else
		{
			digitalWrite(_datasda,LOW);;;
		}
                digitalWrite(_datascl,HIGH);;;
		asm("nop");;;     
                digitalWrite(_datascl,LOW);
		data <<= 1;    
	}
...
void OLED::LED_WrCmd(unsigned char cmd) 
{
	unsigned char i = 8;
        digitalWrite(_datadc,LOW);
	digitalWrite(_datascl,LOW);
         while (i--)
	{
		if (cmd & 0x80)
...
void OLED::LED_Set_Pos(unsigned char x, unsigned char y)
{ 
	LED_WrCmd(0xb0+y);
...
void OLED::LED_Fill(unsigned char bmp_data)
{
	unsigned char y,x;
...
void OLED::LED_Init(void)        
{
	unsigned char i;
        digitalWrite(_datascl,HIGH);;;
        digitalWrite(_datarst,LOW);;; 	
        delay(50);
	//LED_RSTH;;;
        digitalWrite(_datarst,HIGH);
...
void OLED::LED_P6x8Char(unsigned char x,unsigned char y,char ch)
{
...
void OLED::LED_P6x8Str(unsigned char x,unsigned char y,char ch[])
{
...
void OLED::LED_P8x16Str(unsigned char x,unsigned char y,char ch[])
{
	unsigned char c=0,i=0,j=0;
	while (ch[j]!='\0')
	{    
...
void OLED::LED_PrintBMP(unsigned char x0,unsigned char y0,unsigned char x1,unsigned char y1,unsigned char bmp[])
{ 	
	int ii=0;
	unsigned char x,y;
	for(y=y0;y<=y1;y++)
...
void OLED::LED_PrintValueC(unsigned char x, unsigned char y, char data)
{
	unsigned char i,j,k;
	if(data < 0)
	{
...
void OLED::LED_PrintValueI(unsigned char x, unsigned char y, int data)
{
 ...
 void OLED::LED_PrintValueFP(unsigned char x, unsigned char y, unsigned int data, unsigned char num)
 {
 ...
 void OLED::LED_PrintValueF(unsigned char x, unsigned char y, float data, unsigned char num)
 {
 ...
 void OLED::LED_Cursor(unsigned char cursor_column, unsigned char cursor_row)
 {	
 	if(cursor_row != 0) 
 	{
